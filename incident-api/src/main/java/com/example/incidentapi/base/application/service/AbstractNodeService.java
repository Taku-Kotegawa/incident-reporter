...
import com.example.incidentapi.common.datatables.Column;
import com.example.incidentapi.common.datatables.DataTablesInput;
import com.example.incidentapi.common.datatables.Order;
import com.example.incidentapi.common.exception.IllegalStateBusinessException;
import com.example.incidentapi.common.exception.NoChangeBusinessException;
import com.example.incidentapi.common.exception.OptimisticLockingFailureBusinessException;
import com.example.incidentapi.common.message.MessageKeys;
import com.example.incidentapi.common.util.BeanUtils;
...
public abstract class AbstractNodeService<T extends AbstractEntity<ID> & StatusInterface, ID> implements NodeIService<T, ID> {
    ...
    public T save(T entity) {
        ...
        if (!entity.isNew()) {
            ...
            if (current.getStatus().equals(Status.VALID)
                    && entity.getStatus().equals(Status.DRAFT)) {
                currentCopy.setStatus(Status.DRAFT);
            }
...
//        T entity = beanMapper.map(findById(id), clazz);
        T entity = findById(id);
        entityManager.detach(entity);
        if (!entity.getStatus().equals(Status.INVALID)) {
